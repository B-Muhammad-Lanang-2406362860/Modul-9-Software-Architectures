...
The queue chart spiked to the level 5, meaning there is 5 messages that are still on the queue that rabbitmQ cannot yet send to the subscriber because the subscriber is still busy handling previous message (because we simulate that the subscriber must delay 1 second each time receiving a message). As you can see on the lower chart, the deliver ack speed (the speed which rabbitmq deliver the message to the subscriber) is 1 message/second while the consumer ack (the speed which subscriber handle the message and get busy) is only ~0.6 message/second, it means that there's a bottleneck where rabbitMQ can deliver faster but the subscriber isn't ready yet, so the queue message spiked, waiting for the subscriber to not get busy and rabbitmq can send the message again.
![RabbitMQ Chart when the subscriber is slow](assets/04_slow.png)


## Reflection and Running at least three subscribers
As instructed, I ran 3 subscriber that connects and ready to receive message from rabbitmQ server (as you can see there's 3 connection on rabbitMQ dashboard). When I simulate sending 5 messages from publisher, over and over again (just like previous experiment) the message queue did spiked (upper chart), but it come down so quickly to 0 because now there's 3 subscriber that is ready to receive message from the message queue, making lesser delay time for each message. On the lower chart, you can also notice that the purple line (consumer ack) is 2 message/second, it's so much faster then previous experiment (only 0.6 message/second). This is because now we have 3 subscriber that can work simultaneuously rather than just one, hence the rabbitmQ can distribute message more faster since it doesn't wait for one subscriber to finish it.  

![RabbitMQ Chart when the subscriber is slow but there's 3 subscriber](assets/05_improved.png)